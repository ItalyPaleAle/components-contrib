...
func TestGetConfig(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			nr.Metadata{
				Instance: getInstanceInfoWithoutKey(nr.AppID),
				Configuration: map[any]any{
					"SelfRegister": true,
				},
			},
		...
		},
		{
			...
			nr.Metadata{
				Instance: getInstanceInfoWithoutKey(nr.AppPort),
				Configuration: map[any]any{
					"SelfRegister": true,
				},
			},
		...
		},
		{
			...
			func(t *testing.T, metadata nr.Metadata) {
				...
				_, err := getConfig(metadata)
				assert.Error(t, err)
				assert.Contains(t, err.Error(), "DaprHTTPPort")
		...
		},
		{
			...
			func(t *testing.T, metadata nr.Metadata) {
				...
				_, err := getConfig(metadata)
				assert.Error(t, err)
				assert.Contains(t, err.Error(), "DaprInternalPort")
